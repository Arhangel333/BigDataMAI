...
print("=" * 60)
print("СОЗДАНИЕ ОТЧЕТОВ В MONGODB")
print("=" * 60)

# Spark — только для чтения из PostgreSQL
spark = SparkSession.builder \
    .appName("ReportsToMongoDB") \
...
# MongoDB — прямое подключение через pymongo
mongo = MongoClient("mongodb://spark:spark123@mongodb:27017/")
db = mongo["etl_reports"]
...
def save_to_mongo(df, collection_name):
    """Сохраняет DataFrame в MongoDB через pymongo"""
    data = [row.asDict() for row in df.collect()]
    # Конвертируем типы для BSON
    for doc in data:
        for k, v in doc.items():
            if hasattr(v, 'item'):  # numpy типы
                doc[k] = v.item()
    ...
    print(f"   ✅ {collection_name} ({len(data)} документов)")
...
""")
save_to_mongo(top_products, "report_top_products")
...
""")
save_to_mongo(revenue_by_category, "report_revenue_by_category")
...
""")
save_to_mongo(top_customers, "report_top_customers")
...
""")
save_to_mongo(customers_by_country, "report_customers_by_country")
...
""")
save_to_mongo(monthly_trends, "report_monthly_trends")
...
""")
save_to_mongo(top_stores, "report_top_stores")
...
""")
save_to_mongo(popular_products, "report_popular_products")
...
""")
save_to_mongo(product_ratings, "report_product_ratings")
...
""")
save_to_mongo(yearly_revenue, "report_yearly_revenue")
...
""")
save_to_mongo(stores_by_location, "report_stores_by_location")
...
""")
save_to_mongo(top_reviews, "report_top_reviews")
...
top_suppliers = spark.sql("""
    ...
    ORDER BY total_revenue DESC LIMIT 5
""")
save_to_mongo(top_suppliers, "report_top_suppliers")
...
suppliers_by_country = spark.sql("""
    ...
    ORDER BY total_revenue DESC
""")
save_to_mongo(suppliers_by_country, "report_suppliers_by_country")
...
""")
save_to_mongo(rating_sales_corr, "report_rating_sales_corr")

print("\n" + "=" * 60)
print("✅ ВСЕ ОТЧЕТЫ УСПЕШНО СОЗДАНЫ В MONGODB!")
print("=" * 60)

print("\n📊 Коллекции в MongoDB:")
for c in sorted(db.list_collection_names()):
    print(f"   - etl_reports.{c} ({db[c].count_documents({})} док.)")
